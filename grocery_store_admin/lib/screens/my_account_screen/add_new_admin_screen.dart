...
class AddNewAdminScreen extends StatefulWidget {
  ...
  const AddNewAdminScreen({Key key, @required this.admin}) : super(key: key);
  @override
...
class _AddNewAdminScreenState extends State<AddNewAdminScreen> {
  ...
  var image;
  var selectedImage;
  AddNewAdminBloc addNewAdminBloc;
  bool isAdding;
  ...
  void initState() {
    ...
    isAdding = false;
    addNewAdminBloc = BlocProvider.of<AddNewAdminBloc>(context);

    addNewAdminBloc.listen((state) {
      print('ADD NEW DELIVERY BLOC :: $state');

      if (state is AddNewAdminInProgressState) {
        //in progress
        showUpdatingDialog();
      }
      if (state is AddNewAdminFailedState) {
        //failed
        if (isAdding) {
          Navigator.pop(context);
          showSnack('Failed to add new admin!', context);
          isAdding = false;
        }
      }
      if (state is AddNewAdminCompletedState) {
        //completed
        if (isAdding) {
          isAdding = false;
          Navigator.pop(context);
  ...
  addNewAdmin() async {
    if (widget.admin.primaryAdmin) {
      if (_formKey.currentState.validate()) {
        ...
        adminMap.putIfAbsent('activated', () => true);

        addNewAdminBloc.add(
          AddNewAdminEvent(adminMap),
        );
        isAdding = true;
      }
    } else {
