...
class UserDataProvider extends BaseUserDataProvider {
  ...
  Future<List<GroceryUser>> getAllUsers() async {
    ...
    try {
      ...
      users = List<GroceryUser>.from(
        (snap.docs).map(
          (e) {
            return GroceryUser.fromFirestore(e);
          },
        ),
      );
  ...
  Future<bool> addNewDeliveryUser(Map deliveryUserMap) async {
    try {
      ...
      var refundRes = await http.post(
        Uri.parse(
            'https://us-central1-mywashapp-2b43a.cloudfunctions.net/createDeliveryUserAccount'), //TODO: change this URL //it should look something like : https://us-********-**********.cloudfunctions.net/createDeliveryUserAccount
        body: map,
      );
    ...
    try {
      ...
      switch (cancelOrderMap['paymentMethod']) {
        ...
        case 'CARD':
          ...
          var refundRes = await http.post(
            Uri.parse(
                'https://us-central1-mywashapp-2b43a.cloudfunctions.net/createStripeRefund'), //TODO: Change this URL //it should look something like : https://us-********-**********.cloudfunctions.net/createStripeRefund
            body: refundMap,
          );
  ...
  Future<bool> addNewAdmin(Map adminMap) async {
    try {
      ...
      var refundRes = await http.post(
        Uri.parse(
            'https://us-central1-mywashapp-2b43a.cloudfunctions.net/createAdminAccount'), //TODO: change this URL //it should look something like : https://us-********-**********.cloudfunctions.net/createAdminAccount
        body: map,
      );
  ...
  Future<String> sendNewNotification(Map map) async {
    try {
      ...
      var refundRes = await http.post(
        Uri.parse(
            'https://us-central1-mywashapp-2b43a.cloudfunctions.net/sendNewNotification'), //TODO: change this URL //it should look something like : https://us-********-**********.cloudfunctions.net/sendNewNotification
        body: notifMap,
      );
