...
exports.confirmStripePayment = functions.https.onRequest(async(req, res) => {
    try {
        const confirmation = await stripe.paymentIntents.confirm(req.body.id, {
...
//notifications
exports.newOrderNotif = functions.firestore
...
exports.scheduledCheckTrending = functions.pubsub
    ...
    .onRun(async(context) => {
        ...
        console.log("Previous Trending Products :: ", previousTrendingProdsIds);
        console.log("New Trending Products :: ", newTrendingProdsIds);
        return true;
    });

//admin info
exports.updateOrderAnalytics = functions.firestore
    .document("Orders/{orderId}")
    .onUpdate(async(change, context) => {
        ...
        if (
            (change.before.data().orderStatus === "Processing" &&
                change.after.data().orderStatus === "Processed") ||
            change.after.data().orderStatus === "Out for delivery"
        ) {
            ...
            try {
                ...
                await db
                    ...
                    .set({
                        ...
                        processedSales: admin.firestore.FieldValue.increment(
                            parseFloat(change.after.data().charges.totalAmt)
                        ),
            ...
            try {
                await db
                    ...
                    .set({
                        ...
                        cancelledSales: admin.firestore.FieldValue.increment(
                            parseFloat(change.after.data().charges.totalAmt)
                        ),
                        newOrders: newOrders - 1,
                        newSales: newSales - parseFloat(change.after.data().charges.totalAmt),
            ...
            (change.before.data().orderStatus === "Processed" ||
                change.before.data().orderStatus === "Out for delivery") &&
            change.after.data().orderStatus === "Cancelled"
        ) {
            //TODO: add to cancelled
            ...
            try {
                ...
                await db
                    ...
                    .set({
                        ...
                        deliveredSales: admin.firestore.FieldValue.increment(
                            parseFloat(change.after.data().charges.totalAmt)
                        ),
                        processedOrders: processedOrders - 1,
                        processedSales: processedSales -
...
//             console.log('Error sending message:', error);
//         });
exports.updateLowInventoryAnalytics = functions.firestore
    ...
    .onUpdate(async(change, context) => {
        ...
        try {
            if (
                inventoryAfter.quantity < inventoryBefore.quantity &&
                inventoryAfter.quantity <= config.low_inventory_limit &&
                inventoryBefore.quantity > config.low_inventory_limit
            ) {
...
exports.updateCreateCategoryAnalytics = functions.firestore
...
exports.updateDeleteCategoryAnalytics = functions.firestore
    .document("Categories/{categoryId}")
...
exports.updateMessageAnalytics = functions.firestore
    .document("Products/{productId}")
    .onUpdate(async(change, context) => {
...
exports.updateAnsweredMessageAnalytics = functions.https.onRequest(
    async(req, res) => {
        try {
...
exports.updateCreateProductAnalytics = functions.firestore
    .document("Products/{productId}")
    .onCreate(async(snap, context) => {
        try {
...
exports.updateActiveInactiveProductAnalytics = functions.firestore
    .document("Products/{productId}")
    .onUpdate(async(change, context) => {
        const productBefore = change.before.data();
        const productAfter = change.after.data();
        ...
        try {
            if (productBefore.isListed === false && productAfter.isListed === true) {
                ...
                productBefore.isListed === true &&
                productAfter.isListed === false
            ) {
                //product is unlisted
...
exports.updateFeaturedProductAnalytics = functions.firestore
    .document("Products/{productId}")
    .onUpdate(async(change, context) => {
        const productBefore = change.before.data();
        const productAfter = change.after.data();
        ...
        try {
            if (productBefore.featured === false && productAfter.featured === true) {
                ...
                productBefore.featured === true &&
                productAfter.featured === false
            ) {
...
exports.updateChangeCategory = functions.firestore
    .document("Categories/{categoryId}")
    .onUpdate(async(change, context) => {
        ...
        try {
            if (categoryBefore.categoryName != categoryAfter.categoryName) {
                ...
                let snapshots = await db
                    .collection("Products")
                    .where("category", "==", categoryBefore.categoryName)
                    .get();
                ...
                categoryBefore.subCategories.length !=
                categoryAfter.subCategories.length
            ) {
...
exports.updateCreateUserAnalytics = functions.firestore
...
exports.updateBlockedUnblockedUserAnalytics = functions.firestore
    .document("Users/{userId}")
    .onUpdate(async(change, context) => {
        ...
        try {
            if (userBefore.isBlocked === true && userAfter.isBlocked === false) {
                ...
                userBefore.isBlocked === false &&
                userAfter.isBlocked === true
            ) {
                //user is blocked
...
exports.updateActiveInactiveUserAnalytics = functions.firestore
    .document("Users/{userId}")
    .onUpdate(async(change, context) => {
        const userBefore = change.before.data();
        const userAfter = change.after.data();
        ...
        try {
            if (
                userBefore.accountStatus === "Active" &&
                userAfter.accountStatus === "Inactive"
            ) {
                //user is inactive
                ...
                userBefore.accountStatus === "Inactive" &&
                userAfter.accountStatus === "Active"
            ) {
                //user is active
                snap = await db.collection("AdminInfo").doc("userAnalytics").get();
...
exports.updateCreateDeliveryUserAnalytics = functions.firestore
    .document("DeliveryUsers/{userId}")
    .onCreate(async(snap, context) => {
        try {
...
exports.updateDeactivatedDeliveryUserAnalytics = functions.firestore
    .document("DeliveryUsers/{userId}")
    .onUpdate(async(change, context) => {
        const userBefore = change.before.data();
        const userAfter = change.after.data();
        ...
        try {
            if (userBefore.activated === true && userAfter.activated === false) {
                ...
                userBefore.activated === false &&
                userAfter.activated === true
            ) {
                //user is activated
                snap = await db
                    .collection("AdminInfo")
...
exports.updateActiveInactiveDeliveryUserAnalytics = functions.firestore
    .document("DeliveryUsers/{userId}")
    .onUpdate(async(change, context) => {
        const userBefore = change.before.data();
        const userAfter = change.after.data();
...
//notifications
exports.sellerNewOrderNotif = functions.firestore
    .document("Orders/{orderId}")
    .onCreate(async(doc, context) => {
        const orderId = context.params.orderId;
        const orderStatus = doc.data().orderStatus;
...
exports.sellerOrderUpdateNotif = functions.firestore
    .document("Orders/{orderId}")
    .onUpdate(async(change, context) => {
        const orderId = context.params.orderId;
        const orderStatus = change.after.data().orderStatus;
...
exports.userReportNotif = functions.firestore
...
exports.delveryUserOrderAssignedNotif = functions.firestore
    .document("Orders/{orderId}")
    .onUpdate(async(change, context) => {
        const orderId = context.params.orderId;
        const orderStatus = change.after.data().orderStatus;
        ...
        return admin
            ...
            .then(async(queryResult) => {
                ...
                await db
                    .collection("DeliveryUserNotifications")
                    .doc(uid)
                    .set({
                        notifications: admin.firestore.FieldValue.arrayUnion(notificationMap),
                        unread: true,
...
exports.initiateRefundForRazorpay = functions.https.onRequest(
    async(req, res) => {
        try {
...
exports.createDeliveryUserAccount = functions.https.onRequest(
    async(req, res) => {
        try {
            var email = req.body.email;
            var mobileNo = req.body.mobileNo;
...
exports.sendNewNotification = functions.https.onRequest(async(req, res) => {
    try {
        ...
        if (notificationType === "CATEGORY_NOTIF") {
            //add category
            if (imageUrl != null) {
        ...
        var resp = await admin
            .messaging()
            .sendToTopic("ADMIN_PUSH_NOTIFICATIONS", payload);
