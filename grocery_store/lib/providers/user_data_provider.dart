...
//TODO: add the new order values to adminInfo
      ...
      for (var prod in cartList) {
        var price;
        if (prod.product.isDiscounted) {
          price = (1 - (prod.product.discount / 100)) *
              double.parse(prod.sku.skuPrice);
        } else {
          price = double.parse(prod.sku.skuPrice);
        }

        String totalAmt = (price * int.parse(prod.quantity)).toStringAsFixed(2);
        ...
        productsList.add({
          ...
          'unitQuantity': prod.sku.skuName,
          'name': prod.product.name,
          'skuId': prod.sku.skuId,
        });
      ...
      switch (paymentMethod) {
        case 1:
          ...
          //deduct product sku quantities
          for (var prod in cartList) {
            int newQuantity;
            for (var item in prod.product.skus) {
              if (item.skuId == prod.sku.skuId) {
                //found
                newQuantity = item.quantity - int.parse(prod.quantity);
                break;
              }
            }

            await db.collection(Paths.productsPath).doc(prod.product.id).update(
              {
                'skus.${prod.sku.skuId}.quantity': newQuantity,
              },
            );
          }
        ...
        case 2:
          ...
          try {
            var paymentIntentRes = await http.post(
              Uri.parse(
                  'https://us-central1-mywashapp-2b43a.cloudfunctions.net/createPaymentIntent'), //TODO: change this URL //it should look something like : https://us-********-**********.cloudfunctions.net/createPaymentIntent
            ...
            var paymentMethodRes = await http.post(
              Uri.parse(
                  'https://us-central1-mywashapp-2b43a.cloudfunctions.net/createPaymentMethod'), //TODO: change this URL //it should look something like : https://us-********-**********.cloudfunctions.net/createPaymentMethod
              body: json.encode({
            ...
            var paymentConfirmationRes = await http.post(
              Uri.parse(
                  'https://us-central1-mywashapp-2b43a.cloudfunctions.net/confirmStripePayment'), //TODO: change this URL //it should look something like : https://us-********-**********.cloudfunctions.net/confirmStripePayment
              body: payM,
            );
