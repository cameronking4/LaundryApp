...
class CartBloc extends Bloc<CartEvent, CartState> {
  ...
  Stream<CartState> mapEventToState(CartEvent event) async* {
    if (event is AddToCartEvent) {
      yield* mapAddToCartEventToState(event.map);
    }
    ...
    if (event is IncreaseQuantityEvent) {
      yield* mapIncreaseQuantityEventToState(
          event.quantity, event.uid, event.productId, event.id);
    }
  ...
  Stream<CartState> mapAddToCartEventToState(Map map) async* {
    yield AddToCartInProgressState();
    try {
      //add to cart
      bool isAdded = await userDataRepository.addToCart(map);
      if (isAdded) {
  ...
  Stream<CartState> mapIncreaseQuantityEventToState(
    ...
    try {
      bool increased = await userDataRepository.increaseQuantity(
          quantity, uid, productId, id);
      if (increased) {
        // add(GetCartProductsEvent(uid));
